...
with DAG(
    ...
    create_temp = KubernetesPodOperator(
        ...
        image="ubuntu:18.04",
        cmds=["/bin/bash"],
        arguments=["-c","mkdir /airflow && mkdir /airflow/xcom && printf '{\"dir\": \"%s\"}' $(mktemp -d -p /mnt/temp) > /airflow/xcom/return.json && cat /airflow/xcom/return.json"],
    ...
    run_star = KubernetesPodOperator(
        ...
        "--outFileNamePrefix", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/star/",
    ...
    create_salmon_dir = KubernetesPodOperator(
        ...
        image="ubuntu:18.04",
        cmds=["mkdir"],
        arguments=["{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/salmon"],
        volumes=[temp_data_volume],
        volume_mounts=[temp_data_mount],
        resources = {'request_cpu': '50m', 'request_memory': '50Mi'},
        is_delete_operator_pod=True
    ...
    run_salmon = KubernetesPodOperator(
        ...
        "-o", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/salmon"],
        volumes=[input_ref_volume, input_data_volume, temp_data_volume],
        volume_mounts=[input_ref_mount, input_data_mount, temp_data_mount],
        resources = {'request_cpu': '7000m', 'request_memory': '29Gi'},
        is_delete_operator_pod=True
    ...
    create_fastqc_dir = KubernetesPodOperator(
        ...
        image="ubuntu:18.04",
        cmds=["mkdir"],
        arguments=["{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/fastqc"],
        volumes=[temp_data_volume],
        volume_mounts=[temp_data_mount],
        resources = {'request_cpu': '50m', 'request_memory': '50Mi'},
        is_delete_operator_pod=True
    ...
    run_fastqc = KubernetesPodOperator(
        ...
        "-o", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/fastqc",
        "-t", "1"
        ],
        volumes=[input_data_volume, temp_data_volume],
        volume_mounts=[input_data_mount, temp_data_mount],
    ...
    run_samtools = KubernetesPodOperator(
        ...
        arguments=["sort",
        "-n",
        "-o", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/out.sortedByName.bam",
        "-m", "7G",
        "-@", "$(nproc)",
    ...
    create_qualimap_dir = KubernetesPodOperator(
        ...
        image="ubuntu:18.04",
        cmds=["mkdir"],
        arguments=["{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/qualimap"],
        volumes=[temp_data_volume],
        volume_mounts=[temp_data_mount],
        resources = {'request_cpu': '50m', 'request_memory': '50Mi'},
        is_delete_operator_pod=True
    ...
    run_qualimap = KubernetesPodOperator(
        ...
        cmds=["qualimap"], 
        arguments=["rnaseq",
        "-bam", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/out.sortedByName.bam",
        "-gtf", "/mnt/references/ref/gencode.v33.annotation.gtf",
        "--java-mem-size=60G",
        "-pe",
        "-s", "-outdir", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/qualimap"],
    ...
    create_gatk_dir = KubernetesPodOperator(
        ...
        image="ubuntu:18.04",
        cmds=["mkdir"],
        arguments=["{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/tmp"],
        volumes=[temp_data_volume],
        volume_mounts=[temp_data_mount],
        resources = {'request_cpu': '50m', 'request_memory': '50Mi'},
        is_delete_operator_pod=True
    ...
    run_gatk = KubernetesPodOperator(
        ...
        #"-pe",
        "--TMP_DIR", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/tmp"],
        volumes=[temp_data_volume],
        volume_mounts=[temp_data_mount],
    ...
    create_rseqc_dir = KubernetesPodOperator(
        ...
        image="ubuntu:18.04",
        cmds=["mkdir"],
        arguments=["{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/rseqc"],
        volumes=[temp_data_volume],
        volume_mounts=[temp_data_mount],
    ...
    run_rseqc = KubernetesPodOperator(
        ...
        "-i", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/star/Aligned.sortedByCoord.out.bam",
        "-o", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}/rseqc/{{ti.xcom_pull(task_ids = 'parse_filename')}}"],
        volumes=[input_ref_volume, temp_data_volume],
        volume_mounts=[input_ref_mount, temp_data_mount],
        is_delete_operator_pod=True
    )
    ...
    copy_data_to_storage = KubernetesPodOperator(
        ...
        image="ubuntu:18.04",
        cmds=["cp"],
        arguments=["-r", "{{ti.xcom_pull(task_ids = 'create_temp')}}/{{ti.xcom_pull(task_ids = 'parse_filename')}}", "/mnt/output"],
